...
| supersetWebsockets.image.repository | string | `"apache/superset"` | The official websocket image, built from `superset-websocket/` and published as the `-websocket` flavor of the `apache/superset` releases |
| supersetWebsockets.image.tag | string | `"6.1.0-websocket"` | Pin to an immutable release tag (never `latest`) so deployments are reproducible and scannable. Can also be set to `<version>-websocket@sha256:<digest>` to pin by digest. |
