...
### Redis TLS certificate verification enabled by default (behavior change)

When `supersetNode.connections.redis_ssl.enabled` is true, the chart previously defaulted to `ssl_cert_reqs: CERT_NONE`, which told `redis-py` not to validate the Redis server certificate, leaving `rediss://` connections open to man-in-the-middle attacks. The default is `CERT_REQUIRED`.

If your Redis uses a certificate signed by a private CA, supply the CA bundle (mount it into the pods, for example via `extraVolumes`/`extraVolumeMounts`) and set:
...
As an explicit opt-out for self-signed certificates in local development, set `ssl_cert_reqs: CERT_NONE`, which disables server authentication.
...
| supersetNode.connections.redis_ssl.ssl_ca_certs | string | `""` | Path to a CA bundle (mounted into the pods, e.g. via `extraVolumes`) used to validate the Redis server certificate. Leave empty to use the system CAs. |
| supersetNode.connections.redis_ssl.ssl_cert_reqs | string | `"CERT_REQUIRED"` | Certificate verification mode used when `redis_ssl.enabled` is true. Keep `CERT_REQUIRED` so the Redis server certificate is validated. Set to `CERT_NONE` only as an explicit opt-out for self-signed/local development, which disables server authentication and allows MITM on the Redis traffic. |
