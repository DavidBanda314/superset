...
helm install my-superset superset/superset \
  --set supersetNode.connections.db_pass=<your-strong-password> \
  --set postgresql.auth.password=<your-strong-password>
```

The chart no longer ships a default metadata-database password. You must supply
`supersetNode.connections.db_pass` (and, when using the bundled PostgreSQL,
`postgresql.auth.password` set to the same value) or reference an existing
secret via `postgresql.auth.existingSecret`. Rendering fails fast otherwise. See
[Metadata database credentials](#metadata-database-credentials-breaking).
...
### Metadata database credentials (breaking)

Previous versions shipped `supersetNode.connections.db_pass`, `postgresql.auth.password`
and `redis.auth.password` defaulting to the well-known literal `superset`. These
defaults are removed: the values now default to empty and the chart fails to render
if the metadata-database password is not provided.

When upgrading, set the metadata-database password explicitly:

```console
helm upgrade my-superset superset/superset \
  --set supersetNode.connections.db_pass=<your-password> \
  --set postgresql.auth.password=<your-password>   # only for the bundled PostgreSQL
```

To preserve an existing bundled-PostgreSQL release without downtime, pass the password
it already uses. Alternatively, reference an existing secret via
`postgresql.auth.existingSecret` (and `redis.auth.existingSecret`) and supply `DB_PASS`
to Superset through `extraSecretEnv` / `extraEnvRaw` instead of `db_pass`.
...
| supersetNode.connections.db_pass | string | `""` | Metadata database password. Has no default: you must supply it at install time (e.g. `--set supersetNode.connections.db_pass=...` or a values override), matching `postgresql.auth.password` when using the bundled PostgreSQL, or supply `DB_PASS` yourself via `extraSecretEnv` / `extraEnvRaw`. Rendering fails if it is empty and no alternative is provided, to avoid shipping a well-known default credential. |
