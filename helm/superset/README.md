...
| init.adminUser.password | string | `""` | Password for the admin user. Must be set explicitly, otherwise the init job fails. It is published to the init job as the `ADMIN_PASSWORD` environment variable, sourced from the chart's env secret. Alternatively, leave this empty and provide `ADMIN_PASSWORD` yourself through `extraSecretEnv` or `envFromSecret`/`envFromSecrets`. |
