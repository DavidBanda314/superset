...
## Async queries (WebSocket)

When `supersetWebsockets.enabled=true`, you must supply your own
`supersetWebsockets.config.jwtSecret`. The chart ships no default: it is the HS256 key used to
sign and verify the `async-token` cookie, so a shared or publicly known value lets anyone forge a
token for an arbitrary channel and read other users' async query results. Rendering fails when the
value is empty (or left as `CHANGE-ME`). It must match `GLOBAL_ASYNC_QUERIES_JWT_SECRET` in the
Superset config.
...
| supersetWebsockets.config | object | see `values.yaml` | The config.json to pass to the server, see https://github.com/apache/superset/tree/master/superset-websocket Note that the configuration can also read from environment variables (which will have priority), see https://github.com/apache/superset/blob/master/superset-websocket/src/config.ts for a list of supported variables `jwtSecret` has no default and must be set to a strong, unique value matching `GLOBAL_ASYNC_QUERIES_JWT_SECRET` in the Superset config; rendering fails otherwise |
