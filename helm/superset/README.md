...
### Bitnami `redis` subchart 17.9.4 -> 22.0.7

The bundled `redis` subchart was bumped from `17.9.4` (Redis 7.0.10, Debian 11) to `22.0.7` (Redis 8.2.1, Debian 12) to pick up Redis security fixes such as CVE-2024-31449. Superset only uses Redis as a cache and Celery broker, so no data migration is required, but the Redis StatefulSet is recreated on upgrade. If you pin `redis.image.tag` yourself, use a release >= 7.4.2.
...
| oci://registry-1.docker.io/bitnamicharts | redis | 22.0.7 |
