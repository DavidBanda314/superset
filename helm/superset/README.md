...
### Non-root by default (behaviour-changing)

The chart now defaults to running all workloads as the non-root `superset` user (`runAsUser: 1000`) and applies hardened security contexts out of the box: pods set `runAsNonRoot: true` / `fsGroup: 1000` and containers set `allowPrivilegeEscalation: false` with `capabilities.drop: ["ALL"]`. This makes the chart compatible with the `restricted` Pod Security Standard by default.

If your `bootstrapScript` needs root (e.g. to install OS packages at startup), opt back in by overriding the relevant values:
...
For production it is recommended to bake such customizations into your own image instead of running as root.
...
| init.containerSecurityContext.capabilities.drop[0] | string | `"ALL"` |  |
...
| init.podSecurityContext.fsGroup | int | `1000` |  |
...
| resources | object | `{}` |  |
| runAsUser | int | `1000` | User ID directive. This user must have enough permissions to run the bootstrap script Running containers as root is not recommended in production. Defaults to the non-root `superset` user (UID 1000) shipped in the images. Set to 0 only if the bootstrapScript needs to install OS packages as root. |
...
| supersetCeleryBeat.containerSecurityContext.capabilities.drop[0] | string | `"ALL"` |  |
...
| supersetCeleryBeat.podSecurityContext.fsGroup | int | `1000` |  |
...
| supersetCeleryFlower.containerSecurityContext.capabilities.drop[0] | string | `"ALL"` |  |
...
| supersetCeleryFlower.podSecurityContext.fsGroup | int | `1000` |  |
...
| supersetNode.containerSecurityContext.capabilities.drop[0] | string | `"ALL"` |  |
...
| supersetNode.podSecurityContext.fsGroup | int | `1000` |  |
...
| supersetWebsockets.containerSecurityContext.capabilities.drop[0] | string | `"ALL"` |  |
...
| supersetWebsockets.podSecurityContext.fsGroup | int | `1000` |  |
...
| supersetWorker.containerSecurityContext.capabilities.drop[0] | string | `"ALL"` |  |
...
| supersetWorker.podSecurityContext.fsGroup | int | `1000` |  |
