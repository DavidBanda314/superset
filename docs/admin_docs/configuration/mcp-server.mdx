...
Dev-mode auth executes every request as this user with no credential, so the
server refuses to start with `MCP_DEV_USERNAME` set unless the deployment is a
development one (`SUPERSET_ENV=development` or Flask debug) bound to a loopback
address, MCP transport auth is configured, or you explicitly set
`MCP_ALLOW_DEV_AUTH = True`.
...
Development mode is guarded at startup: when `MCP_DEV_USERNAME` is set and no
transport auth provider is configured, the MCP server raises
`MCPAuthConfigError` and refuses to start unless

- `SUPERSET_ENV` is `development`/`debug` (or the Flask app runs in debug mode), **and**
- `--host`/`MCP_SERVICE_HOST` is a loopback address (`127.0.0.1`, `::1`, `localhost`).

Set `MCP_ALLOW_DEV_AUTH = True` to override the guard and accept the risk of
serving unauthenticated requests as the configured user. Every startup with
dev-mode auth active logs a `WARNING` naming the impersonated user.
...
| `MCP_DEV_USERNAME` | -- | Superset username for development mode (no auth). Only allowed for a loopback-bound development deployment unless `MCP_ALLOW_DEV_AUTH` is `True` |
| `MCP_ALLOW_DEV_AUTH` | `False` | Opt in to `MCP_DEV_USERNAME` dev-mode auth outside a loopback-bound development deployment. Leaving this `False` makes the server fail to start rather than serve unauthenticated requests as a privileged user |
