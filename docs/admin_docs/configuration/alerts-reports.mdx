...
Superset ships with no SMTP credentials: `SMTP_USER` and `SMTP_PASSWORD` default to the `SMTP_USER`
and `SMTP_PASSWORD` environment variables and to the empty string when those are unset, in which
case the SMTP login step is skipped. Set them in `superset_config.py` as shown above, or supply them
through the environment to keep the credentials out of your configuration file.
