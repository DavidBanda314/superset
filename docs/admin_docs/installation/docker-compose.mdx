...
The metadata database credentials (`DATABASE_PASSWORD`, `POSTGRES_PASSWORD` and `EXAMPLES_PASSWORD`)
are shipped empty in `docker/.env` and the stack will not start until they are provided. Generate
them with `./scripts/generate-docker-secrets.sh` (also run automatically by `make up`), or set them
yourself in `docker/.env-local` using something like `openssl rand -base64 24`. `DATABASE_PASSWORD`
and `POSTGRES_PASSWORD` must be set to the same value.
