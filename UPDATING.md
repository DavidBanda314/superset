...
- **Breaking — async-query JWT cookie hardened by default.** `GLOBAL_ASYNC_QUERIES_JWT_COOKIE_SECURE` now defaults to `True` and `GLOBAL_ASYNC_QUERIES_JWT_COOKIE_SAMESITE` now defaults to `"Lax"` (previously `False` and `None`). The `async-token` cookie is therefore only sent over HTTPS and is not attached to cross-site requests. Deployments serving Superset over plain HTTP must set `GLOBAL_ASYNC_QUERIES_JWT_COOKIE_SECURE = False` explicitly, and embedded / cross-origin deployments that need the cookie on cross-site requests must set `GLOBAL_ASYNC_QUERIES_JWT_COOKIE_SAMESITE = "None"` together with `Secure` (browsers reject `SameSite=None` without it).
