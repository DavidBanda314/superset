...
- **Rate limiting enabled by default.** `RATELIMIT_ENABLED` no longer depends on `SUPERSET_ENV == "production"`; it defaults to `True` so login brute-force protection is active in every deployment. Operators who do not want Flask-Limiter must set `RATELIMIT_ENABLED = False` explicitly. `AUTH_RATE_LIMIT` was tightened from `"5 per second"` to `"5 per minute;20 per hour"`. Deployments running multiple web workers should set `RATELIMIT_STORAGE_URI` to a shared backend (e.g. Redis), otherwise the in-memory counters are per process.
