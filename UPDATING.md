...
- **Secure session cookie / transport defaults.** `SESSION_COOKIE_SECURE` now defaults to `True` (overridable via the `SESSION_COOKIE_SECURE` environment variable), and the production `TALISMAN_CONFIG` now sets `force_https: True`, `session_cookie_secure: True`, and `strict_transport_security: True`. The dev config (`TALISMAN_DEV_CONFIG`, used automatically in debug mode) keeps the previous insecure values so local development over plain HTTP is unaffected. Operators serving Superset over plain HTTP in production must either front it with HTTPS (recommended) or explicitly set `SESSION_COOKIE_SECURE=False`; a startup warning is emitted when `SESSION_COOKIE_SECURE` is disabled outside development.
