...
- **Default SMTP credentials removed.** `SMTP_USER` and `SMTP_PASSWORD` in `superset/config.py` no longer default to the committed `superset` / `superset` credential pair; they default to the `SMTP_USER` / `SMTP_PASSWORD` environment variables and fall back to the empty string, which makes the mailer skip the SMTP login step. Deployments that relied on the shipped defaults must set the credentials explicitly in `superset_config.py` or via those environment variables.
