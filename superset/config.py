...
#: Defense-in-depth against man-in-the-middle attacks on SSH tunnels: when enabled,
#: every SSH tunnel must declare an expected server host key (``server_host_key`` on
#: the tunnel) and the SSH server's presented host key is verified against it before
#: the tunnel is opened. A mismatch, or a missing expected key while this flag is
#: enabled, fails closed and the tunnel is rejected. This defaults to ``True`` so
#: tunnels are not opened without host-key verification (paramiko's ``Transport`` does
#: no known-hosts checking by default). When disabled, tunnels without a
#: ``server_host_key`` open without host-key verification; tunnels that do set a
#: ``server_host_key`` are still verified regardless of this flag.
SSH_TUNNEL_STRICT_HOST_KEY_CHECKING: bool = True
